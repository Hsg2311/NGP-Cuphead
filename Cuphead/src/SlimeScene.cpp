...
void SlimeScene::handleAttackPacket(const Packet& packet)
{
	Player* obj = reinterpret_cast<Player*>(PacketQueue::getInst().getObject(packet.ar.id));
	switch (packet.ap.dir) {
	case Direction::SW:
		obj->createProjectile();
	}
}
